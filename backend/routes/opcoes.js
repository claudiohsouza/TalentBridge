...
// Rota para obter todas as opções disponíveis
router.get('/', authMiddleware, async (req, res, next) => {
  try {
    ...
    const opcoesQuery = await pool.query(`
      SELECT categoria, json_agg(valor) as valores
      FROM opcoes_sistema
      GROUP BY categoria
      ORDER BY categoria
    `);
    
    // Transformar resultado em objeto de fácil uso
    const opcoes = {};
    opcoesQuery.rows.forEach(row => {
      opcoes[row.categoria] = row.valores;
    });
    
    res.json(opcoes);
  } catch (error) {
    console.error('[API-opcoes] Erro ao buscar opções:', error);
    next(error);
  }
});

// Rota para obter opções de uma categoria específica
router.get('/:categoria', authMiddleware, async (req, res, next) => {
  try {
...
// Endpoint para iniciar/popular a tabela de opções (apenas para desenvolvimento/administração)
router.post('/init', authMiddleware, async (req, res, next) => {
  // Verificar se o usuário é administrador
  if (req.user.papel !== 'administrador' && req.user.email !== 'admin@example.com') {
