...
const DashboardChefeEmpresa: React.FC = () => {
  ...
  const handleNovoJovem = () => {
    navigate('/chefe-empresa/jovens/novo');
  };
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
          <div className="card p-6 hover:border-cursor-primary transition-colors duration-300">
            <h2 className="text-lg font-semibold text-cursor-text-primary mb-2">Jovens da Empresa</h2>
        ...
        <div className="card overflow-hidden">
          <div className="p-6 border-b border-cursor-border flex justify-between items-center">
            <h2 className="text-lg font-semibold text-cursor-text-primary">Jovens da Empresa</h2>
            <button 
                  ...
                  <tbody className="divide-y divide-cursor-border">
                    {jovens.slice(0, 5).map(jovem => (
                      <tr key={jovem.id} className="hover:bg-cursor-background-light transition-colors">
                        ...
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-cursor-text-secondary">
                          {jovem.empresas && jovem.empresas.length > 0 ? jovem.empresas[0].cargo : '-'}
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap text-sm">
                          {jovem.empresas && jovem.empresas.length > 0 ? (
                            <span className={`badge ${
                              jovem.empresas[0].status === 'Contratado' ? 'badge-success' : 
                              jovem.empresas[0].status === 'Estagiário' ? 'badge-warning' : 
                              'badge-default'
                            }`}>
                              {jovem.empresas[0].status}
                            </span>
                          ) : '-'}
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap text-sm">
